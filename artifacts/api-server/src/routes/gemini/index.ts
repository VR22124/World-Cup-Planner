...
import { streamGeminiResponse } from "../../services/aiService.js";
import {
  getStadiumStatus,
  getIncidents,
  getAlerts,
...
function buildSystemPrompt(): string {
  ...
  const alerts = getAlerts().filter((a) => !a.acknowledged && a.level === "critical");
...
- Match: ${status.matchStatus.homeTeam} ${status.matchStatus.homeScore}-${status.matchStatus.awayScore} ${status.matchStatus.awayTeam} | Minute ${status.matchStatus.minute} (${status.matchStatus.phase.replace("_", " ")})
- Attendance: ${status.totalAttendance.toLocaleString()} fans (${status.capacityPercent}% capacity)
- Crowd: ${status.overallCrowdLevel.toUpperCase()}
- Weather: ${status.weather.condition}, ${status.weather.temperatureCelsius}°C
